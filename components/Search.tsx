        ...
        {open && 
          <ul className='search-result'>{
            results.length >0 ? results.map((file)=>(
              <li key={file.$id} onClickCapture={()=>handleClickItem(file)} className='flex items-center justify-between' onClick={()=>handleClickItem(file)}>
              <div className='flex cursor-pointer items-center gap-4'><Thumbnail type={file.type} extension={file.extension} url={file.url} className='size-9 min-w-9'/><p className='subtitle-2 line-clamp-1 text-light-100'>{file.name}</p></div>
