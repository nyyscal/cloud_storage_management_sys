...
<div className='header-user'>
  <Image className='header-user-avatar' src={avatar} alt="avatar" width={44} height={44}/>
  <div className='sm:hidden lg:block'><p className='subtitle-2 capitalize'>{fullName}</p>
  <p className='caption'>{email}</p></div>
</div>
<Separator className='mb-4 bg-light-200/20'/>

<nav className='mobile-nav'>
  <ul className='mobile-nav-list'>{navItems.map(({url,name,icon})=>(
...
<Button onClick={async()=>await signOutUser()} className='mobile-sign-out-button' type="submit"><Image src="/assets/icons/logout.svg" alt="logo" width={24} height={24} /><p>Logout</p></Button>
</div>
  </SheetContent>
</Sheet>
