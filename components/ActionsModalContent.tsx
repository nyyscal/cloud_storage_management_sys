        ...
        <div className='pt-4'>
          ...
          <ul className='pt-2'>{file.users.map((email:string)=>(
            <li key={email} className='flex items-center justify-between gap-2'>
              ...
              <Button onClick={()=>onRemove(email)} className='share-remove-user'><Image src="/assets/icons/remove.svg" width={24} height={24} className='remove-icon' alt="Remove"/></Button>
            </li>
          ))}</ul>
          </div>
      </div>
